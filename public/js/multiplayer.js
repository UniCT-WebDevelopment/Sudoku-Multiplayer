...
const socket = io();
let enemy = null;
let enemysUsername = $('#enemys-username');
const cover = $('#cover');
...
socket.on('connect', () => {
    ...
    socket.emit('startMultiplayerGame', data);
    socket.emit('playerReady');
})

socket.on('otherPlayerHasJoined', () => {
    socket.emit('startOnlineGame');
})

socket.on('startPlaying', (data) => {
    enemy = data.enemy;
    enemysUsername.html(`You're going up against ${enemy}`)
    ...
    for (const offset of offsets) {
        for(let i = 0; i < 3; i++) {
            for(let k = 0; k < 3; k++) {
                for(let j = 0; j < 3; j++) {
                    const digit = data.board[index++];
                    if(digit > 0) {
                        elements[offset + i*3 + k*9 + j].innerHTML = digit;
                        $(elements[offset + i*3 + k*9 + j]).addClass('correct-solution');
                    } else {
                        elements[offset + i*3 + k*9 + j].innerHTML = '&nbsp;'
                    }
                }
            }
        }
...
socket.on('won', (data) => {
    pauseButtonFunction();
    let message;
    if(grid.find('.correct-solution').length + grid.find('.solution').length < 81) {
        message = (`${enemy} has forfeited`);
    } else {
        message = `You defeated ${enemy}`
    }
    ...
    cover.html(`
            <div class="container">
                ...
                <div class="row">
                    <div class="col-6 d-flex">${data.gamesWon ? `Total games won: `: ''}</div>
                    <div class="col-6 d-flex justify-content-end">${data.gamesWon ? data.gamesWon: ''}</div>
                </div>
            </div>
        `)
...
socket.on('penalty', () => {
    journal = [];
    journalCount = 0;
    const solutions = grid.find('.solution');
    if(solutions.length == 0) return;
    removeAllSelectedClasses();
    removeAllHighLight();
    const randomElementIndex = Math.floor(Math.random() * solutions.length);
    const randomSolution = solutions[randomElementIndex];
    randomSolution.dispatchEvent(new MouseEvent('mousedown', {
        bubbles: true,
        cancelable: true,
        view: window
    }));
    randomSolution.dispatchEvent(new MouseEvent('mouseup'), {
        bubbles: true,
        cancelable: true,
        view: window
    });
    removeSolution();
    enemysUsername.html(`${enemy} has removed one of your solutions`);
    setInterval(() => {
        enemysUsername.html(`You're going up against ${enemy}`)
    }, 4000)
})
    ...
    // if(correctSolution.length + solution.length == 81) {
        ...
        for (const offset of offsets) {
            for(let i = 0; i < 3; i++) {
                for(let k = 0; k < 3; k++) {
                    for(let j = 0; j < 3; j++) {
                        const digit = Number(elements[offset + i*3 + k*9 + j].innerHTML);
                        if(digit > 0 && digit < 10) sudoku += digit;
                        else sudoku += 0;
                    }
                }
            }
